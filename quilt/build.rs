...
fn build(lang: &str) {
    ...
    // These parsers/scanners are tree-sitter-generated, vendored verbatim from
    // the pinned forks (regenerated by `bin/sync-grammars`), so we never act on
    // their warnings — e.g. typescript's scanner trips `-Wunused-parameter`.
    // Don't add `-Wall`/`-Wextra`; they only flood the build log as
    // `cargo:warning=` lines for third-party code we don't maintain.
    cc.warnings(false);
