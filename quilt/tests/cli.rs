...
/* ── notebook: an .html.quilt page whose cells run on machines ─────────── */

/// A small notebook whose cells need only `python3` and `bash`: a page
/// element, a Python cell reading it, a bash cell reading it.
const NOTEBOOK: &str = "<h1 id=\"n\">20</h1>\n\
    <div class=\"row\">\n  py↖int(↙#n↘) + 1↗\n  bash↖echo \"n is ↙#n↘\"↗\n</div>\n";

/// `quilt notebook` writes the rendered page beside the source — the input
/// name without `.quilt` — with a generated-file header, the cells' results
/// in it, and a summary on stderr.
#[test]
fn notebook_writes_the_sibling_page() {
    let d = Dir::new("notebook");
    let f = d.write("nb.html.quilt", NOTEBOOK);
    let o = run(&[Path::new("notebook"), &f]);
    assert!(o.status.success(), "stderr:\n{}", stderr(&o));
    let page = std::fs::read_to_string(d.0.join("nb.html")).expect("nb.html written");
    assert!(
        page.starts_with("<!-- DO NOT EDIT. GENERATED BY `quilt notebook"),
        "{page}"
    );
    assert!(
        page.contains("<pre class=\"quilt-value\">21</pre>"),
        "{page}"
    );
    assert!(
        page.contains("<pre class=\"quilt-stdout\">n is 20</pre>"),
        "{page}"
    );
    assert!(stderr(&o).contains("2 cell(s), 0 failed"), "{}", stderr(&o));
}

/// `-o` picks the output; `--stdout` prints the page instead of writing.
#[test]
fn notebook_output_options() {
    let d = Dir::new("notebook-out");
    let f = d.write("nb.html.quilt", NOTEBOOK);
    let out = d.0.join("elsewhere.html");
    let o = run(&[Path::new("notebook"), &f, Path::new("-o"), &out]);
    assert!(o.status.success(), "stderr:\n{}", stderr(&o));
    assert!(out.exists());
    assert!(!d.0.join("nb.html").exists());

    let o = run(&[Path::new("notebook"), &f, Path::new("--stdout")]);
    assert!(o.status.success(), "stderr:\n{}", stderr(&o));
    assert!(stdout(&o).contains("<pre class=\"quilt-value\">21</pre>"));
    assert!(!d.0.join("nb.html").exists());
}

/// `expand` on an HTML-ground file renders the notebook (the identity
/// expansion is nothing anyone wants written), and `run` prints it — so a
/// `#!/usr/bin/env quilt` notebook is a script whose output is its page.
#[test]
fn expand_and_run_render_a_notebook() {
    let d = Dir::new("notebook-expand");
    let f = d.write("nb.html.quilt", NOTEBOOK);
    let o = run(&[Path::new("expand"), &f]);
    assert!(o.status.success(), "stderr:\n{}", stderr(&o));
    let page = std::fs::read_to_string(d.0.join("nb.html")).expect("nb.html written");
    ...
    let script = d.write(
        "script.html.quilt",
        &format!("#!/usr/bin/env quilt\n{NOTEBOOK}"),
    );
    let o = run(&[Path::new("run"), &script]);
    assert!(o.status.success(), "stderr:\n{}", stderr(&o));
    assert!(
        stdout(&o).contains("<pre class=\"quilt-value\">21</pre>"),
        "{}",
        stdout(&o)
    );
    assert!(!d.0.join("script.html").exists(), "run writes nothing");
}

/// `check` validates every cell with its own grammar and runs none of them.
#[test]
fn check_validates_a_notebook_without_running_it() {
    let d = Dir::new("notebook-check");
    let marker = d.0.join("ran");
    let f = d.write(
        "nb.html.quilt",
        &format!("<p>bash↖touch \"{}\"↗ py↖x = 1↗</p>\n", marker.display()),
    );
    let o = run(&[Path::new("check"), &f]);
    assert!(o.status.success(), "stderr:\n{}", stderr(&o));
    assert!(!marker.exists(), "check must not run cells");

    let bad = d.write("bad.html.quilt", "<p>py↖def oops(:↗</p>\n");
    let o = run(&[Path::new("check"), &bad]);
    assert!(!o.status.success(), "a cell's syntax error fails check");
}

/// A failed cell is rendered, not fatal; `--strict` makes it the exit code.
#[test]
fn notebook_strict_reports_failed_cells() {
    let d = Dir::new("notebook-strict");
    let f = d.write("nb.html.quilt", "<p>py↖undefined_name↗ py↖2 + 2↗</p>\n");
    let o = run(&[Path::new("notebook"), &f, Path::new("--stdout")]);
    assert!(o.status.success(), "stderr:\n{}", stderr(&o));
    assert!(stdout(&o).contains("quilt-failed"));
    assert!(stdout(&o).contains("<pre class=\"quilt-value\">4</pre>"));
    assert!(stderr(&o).contains("cell 1 [py] failed"), "{}", stderr(&o));
    ...
    assert!(!o.status.success());
    assert!(stderr(&o).contains("1 cell(s) failed"), "{}", stderr(&o));
}

/// A notebook is an HTML-ground file; anything else is refused by name.
#[test]
fn notebook_rejects_another_ground_language() {
    let d = Dir::new("notebook-ground");
    let f = d.write("prog.py.quilt", "print(1)\n");
    let o = run(&[Path::new("notebook"), &f]);
    assert!(!o.status.success());
    assert!(stderr(&o).contains("ground language"), "{}", stderr(&o));
}

/// `quilt repl html` is a notebook session: markup defines, quotes run,
/// `#id` reads, and the page is printed at the end.
#[test]
fn repl_html_is_a_notebook_session() {
    ...
    child
        ...
        .take()
        .unwrap()
        .write_all(b"<p id=\"x\">5</p>\npy\xe2\x86\x96int(\xe2\x86\x99#x\xe2\x86\x98) * 2\xe2\x86\x97\n#x\n")
        .unwrap();
    let o = child.wait_with_output().expect("runs");
    assert!(o.status.success(), "stderr:\n{}", stderr(&o));
    let s = stdout(&o);
    assert!(s.starts_with("10\n<p id=\"x\">5</p>\n"), "{s}");
    assert!(s.contains("<figure class=\"quilt-cell quilt-py\""), "{s}");
}
...
fn help_lists_the_subcommands() {
    ...
    assert!(o.status.success());
    let s = stdout(&o);
    for sub in ["expand", "check", "run", "notebook"] {
