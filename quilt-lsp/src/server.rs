...
impl LanguageServer for Backend {
    ...
    async fn did_change(&self, params: DidChangeTextDocumentParams) {
        ...
        // Apply the edits to our copy of the text. There is no tree to edit
        // alongside it: re-scanning the whole buffer costs ~5 µs/KiB, so the
        // `InputEdit` bookkeeping tree-sitter needed to reuse subtrees cost
        // more to maintain than it saved (issue #254).
        let mut text = match self.inner.docs.get(&uri) {
        ...
        self.inner.ingest(uri, text, version).await;
    }
