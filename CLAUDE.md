...
# Notebooks (docs/wiki/notebook.md): an .html.quilt page whose quoted cells
# (`py↖…↗`, `sql↖…↗`, `bash↖…↗`, `ts↖…↗`) run on the machines of their
# languages, with the page itself as the HTML machine. `expand` and `run` on an
# HTML-ground file render the notebook too; `check` validates every cell's
# syntax without running one. Python cells that quote HTML need `bin/build-py`.
quilt notebook path/to/notes.html.quilt            # writes notes.html beside it
...
`↑` is target-directed: `MetaLanguage::lift_str(target)` picks the spelling, where `target` defaults to the language of the enclosing quote (threaded through `build_nodes` as `splice_target`). Rust's spellings (`langs::rust::ops::lift_spelling`) are `qlift()` for rust→rust and `qlift_to::<L>()` for the heterogeneous targets (python, wgsl, zsh, bash, nix, lean, sql/mysql). `lift.rs` (always compiled, no parser deps — wasm consumers use it) defines `LiftTo<L>` keyed by marker types (`Rust`, `Python`, `Wgsl`, `Bash`, `Zsh`, `Nix`, `Lean`, `Sql`, `MySql`) plus the `QLiftTo` postfix helper; per-(type, language) impls own the target's tags and spellings (e.g. `LiftTo<Wgsl> for u32` → `leaf("int_literal", "3u")`, `LiftTo<Python> for Vec<T>` → a `list` literal). Those impls live in `lift/gen.rs` and are **generated** (issue #203): `bin/gen-lifts` runs `lift/mk_lifts.rs.quilt`, which parses a sample literal per family with the vendored grammar, lifts it with `↑` — the builder-call source that rebuilds it — and substitutes the sample's text for the runtime expression, so the tags and layout are the parser's rather than a transcription. `bin/check-lifts` is the drift gate. `lift/mod.rs` keeps what is not a shape: markers, the trait, and the `*_dquote_escape` / `sql_squote_escape` rules the generated shapes call.
...
**Target-only languages** (wgsl, sql) provide just `lang.rs` — they can be quoted (`wgsl↖...↗`) but have no `MetaLanguage`, so the host's meta drives expansion. **HTML** is a quotable target *and* an **identity host**: `langs/html/meta.rs` is the same `HoldMetaLanguage` text's meta is (`langs/text/meta.rs`), so an `.html.quilt` file expands to itself with its quoted cells held verbatim — which is what lets `quilt check` validate a notebook's cells without running one. The computation lives in the machine instead: `quilt/src/notebook.rs` (`quilt notebook`) plants each cell as a placeholder element, holds the page as the native **`HtmlMachine`** (`quilt/src/machine/html.rs` — a document whose *definitions are its ids*: feeding an element with a known id replaces it, `#id` queries it back, the typing judgment is the tag name), re-parses each cell as a program of its language, expands it with that language's meta, feeds it to the language's park machine, and defines the rendered `<figure>` back into the page. Output with an already-defined `id` redefines that element wherever it is (cells edit the page); `↙#id↘` inside a cell is a document reference, spliced as a string literal of the cell's language (the page is how machines that share nothing exchange values); output is parsed as Quilt-in-HTML, so a quote a cell prints is a new cell (cells create cells, capped at `MAX_GENERATION`). See `docs/wiki/notebook.md` and `examples/notebook/tour.html.quilt`. **SQL** additionally carries the point of the exercise in its `LiftTo` impls (issue #219): a value spliced with `↑` becomes a `literal` *node*, spelled as standard SQL with every `'` doubled, so it cannot close the literal and continue the statement — see `lift::sql_squote_escape`. `mysql`/`mariadb` are the same grammar under a different annotation, differing only in that their escaper also doubles the backslash MySQL reads as an escape and the standard does not (issue #233) — no single spelling is right for both, so the dialect is annotated rather than guessed. Its `program` holds statements, so a bare expression fragment (`sql↖id = 1↗`) reaches a `QTerm` only through the `SELECT …` wrapper retry in `parse_pre`, the same technique Lean uses with `#check …`.
...
`langs/omni.rs` defines `Omni` (the default `Multi` used by the CLI) using enum-dispatch over all enabled languages. Registry keys: `rust`/`rs`, `python`/`py`, `text`/`txt`, `wgsl`, `html`, `zsh`, `bash`, `nix`, `lean`/`lean4`, `sql`/`mysql`/`mariadb`, `typescript`/`ts`.

### Machines (`quilt/src/machine.rs`, `docs/design/machines.md`)
