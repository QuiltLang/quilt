...
**Target-only languages** (wgsl, html, zsh, bash) provide just `lang.rs` — they can be quoted (`wgsl↖...↗`) but have no `MetaLanguage`, so the host's meta drives expansion. **Text** additionally has a `meta.rs`, but it is not a working host: text has no grammar to write a metaprogram in and no runtime to build terms with, so every method returns an explanatory error. It is absent from `omni.rs`'s `metas` section and therefore unreachable through `Omni`.
