...
| 2 | [Builder-call emitters](#2-builder-call-emitters-langsops) | `langs/chain/`: one fold, plus a generated fragment table | **Done** — `bin/gen-chain` ([#204](https://github.com/QuiltLang/quilt/issues/204)); the fold turned out to be cloned four times, not three |
...
The fold lives once, in `quilt/src/langs/chain/mod.rs`; the fragments are a
`Shapes` table in `chain/gen.rs`, generated by `bin/gen-chain` from
`chain/mk_chain.rs.quilt`, with `bin/check-chain` as the drift gate in `main
check` and in CI beside `check-lifts`. Each `ops.rs` keeps a `const CHAIN` that
pairs its table with its escaper, and the four `build_*_code` entry points become
one line each.

The survey said three copies of the fold. There were **four**: `QLift for
Arc<QTerm>` in `rust/ops.rs` is the same fold again, emitting structured
`string_literal` subterms instead of flat text so that `rewrite_naive` can match
what it produces. That difference is now a mode — `Lit::Flat` against
`Lit::Term` — rather than a second copy, which also collapsed `cmds_lit` and
`cmds_lit_term` into one function.

What the samples settle, and what they do not, is worth being exact about:

* **The two borrows are settled by the parser**, as hoped. `py↖ACC.c(&LIT)↗` and
  `py↖&[EL1, EL2]↗` are both parse errors, so a fragment that carries a Rust
  borrow into Python or TypeScript fails generation instead of shipping.
* **`NL` against `NL()` is not.** Both are valid TypeScript, because that
  divergence is a fact about the `quilt-wasm` runtime (wasm-bindgen cannot export
  a module-scope constant) rather than about the syntax. It is still a claim —
  now a claim made in one place, next to the two the parser does check.
* **Escaping stays hand-written** in each `ops.rs`, for the same reason it did in
  finding 1: which characters a literal must escape is runtime logic, not a shape
  a sample can carry.

Rust's variadic node stays hand-written too, and is the one place the three
languages genuinely differ rather than differing in spelling: Python and
TypeScript splice into a variadic container with a fluent `.e()` chain, so they
share the fold with a different hole fragment, while Rust emits an imperative
`{ let mut b_ = …; …; b_.b() }` block — which is not a chain, and is what makes
statement-context splicing possible in Rust and not in the other two.

Only the *text* is contractual: the emitted terms are flat, `"_"`-tagged layout,
serialized with `coparse` and never matched against, so the shared fold groups
its writes differently from the four folds it replaced. That the text did not
move was checked four ways — the 49 expander snapshots, `expand_both`
(`RustMetaLanguage` against the independent `BootstrapMetaLanguage`, which is
deliberately *not* built on this fold), `check-bootstrap` leaving the generated
`rust/meta.rs` byte-identical, and expanding all 28 `examples/` with the binary
from before and after and diffing.
