...
- `--prelude <LINE>` — open the generated file with `LINE` instead of the host's runtime import (see below).
- `--no-prelude` — generate no runtime import at all.

#### The runtime import (issue #274)

Expanded code *calls a runtime*: `tb(..)`, `qlift(..)`, `name(..)` are names
that have to be in scope. `expand` opens the generated file with the import that
brings them there, so a `.quilt` source does not have to carry it by hand:

| Ground language | Injected                                                    |
|-----------------|-------------------------------------------------------------|
| Rust            | `use quilt::prelude::*;`                                    |
| Python          | `from quilt import *`                                       |
| TypeScript      | `import { tb, leaf, …, qlift, … } from "quilt";`            |

The spelling comes from the host's meta-language (`MetaLanguage::prelude`), not
from the CLI — see [Language traits](language-traits.md#prelude--the-runtime-import).
A string-based host (nix, lean, text) generates no runtime calls and so gets
nothing.

TypeScript's is the interesting one: it has no glob import, so the list has to
name every runtime function the expansion calls — and the lift spelling is
*target-directed* (`↑` into TypeScript is `qlift`, into HTML `qlift_html`), so
which languages the file quotes decides what is in it. The targets are the
file's language chain plus every annotation its quotes name, so a `html↖…↗`
inside a plain `.ts.quilt` file is counted too.

Three rules keep it out of the way:

- **Only when needed.** A file that uses no quote, unquote or operator glyph
  expands to itself and is left import-free. A Quilt *comment* does not count —
  it is deleted, not translated into a call.
- **Never twice.** A source that already brings the runtime in keeps its own
  import. Two glob imports are harmless in Rust and Python, but a doubled
  *named* import is a hard `Duplicate identifier` error in TypeScript.
- **An escape hatch.** The path is not universal: code generated *inside* this
  repo wants `use crate::prelude::*;` (`--prelude`), and a runtime that
  re-expands a generated stage — quilt-python's `expand()`, quilt-wasm's — wants
  nothing at all (`--no-prelude`), because it evaluates the result in a scope
  that already holds the runtime.

The import is prepended to the *file*, not built into the term, which is why
`quilt check` stays a pure parse-and-expand and the expander snapshots do not
carry it.

The generated file starts with a comment `//! DO NOT EDIT. GENERATED BY quilt expand ...`. The marker is the one the *generated* language uses, not always Rust's — `#` for Python, `--` for Lean — asked of the language registry (`langs::header_comment`, backed by each language's `Comments::HEADER`), keyed by the output file's extension.

A leading `#!` shebang is **dropped**. It says how to run the `.quilt` *file* — it is what makes `./examples/countdown.rs.quilt` and the extension-less `bin/issues` work — and it says nothing about the artifact: `run` already ignores it (the interpreter comes from `Language::hashbang()`, the language's answer, not the script's line) and strips it before expanding, and `check` blanks it. `expand` used to copy it through, where the header had already taken line 1 so it was not a shebang at all, and where in Rust it was worse than noise: `#!` there is inner-attribute syntax, so every generated file that came from an executable script failed to parse. An inner attribute is not a shebang and is kept — `#!` is one only when the next character is not `[`, exactly the rule rustc reads it by.
...
`run` injects the same [runtime import](#the-runtime-import-issue-274) as
`expand`, and takes the same `--prelude` / `--no-prelude` flags (before the
filename, like `-m`). This is the injection that matters most: the temp file is
handed straight to an interpreter, so a missing import is a failed *run* rather
than a stale artifact someone notices later.
